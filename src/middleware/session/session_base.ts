...
import { Context } from 'npm:telegraf@4.12.3-canary.1/src/context.ts'
import { MaybePromise } from 'npm:telegraf@4.12.3-canary.1/src/util.ts'
import { MiddlewareFn } from 'npm:telegraf@4.12.3-canary.1/src/middleware.ts'
import d from 'npm:debug@4.3.4'
...
> {
  ...
  property?: Property
  closeSessionPromise?: (ctx: C) => Promise<unknown>
}
...
>(options?: SessionOptions<S, C, P>): MiddlewareFn<C & { [session in P]?: S }> {
  ...
  const closeSessionPromise = options?.closeSessionPromise ?? (() => Promise.resolve())
  ...
  return async (ctx: C, next: () => any) => {
    ...
    try {
      ...
      closeSessionPromise(ctx).finally(async () => {
        if (--c.counter === 0) {
